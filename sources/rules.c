...
od_rules_storage_copy(mcxt_context_t mcxt, od_rule_storage_t *storage)
{
	...
	if (storage->stolon_config.store_prefix) {
		copy->stolon_config.store_prefix = mcxt_strdup(storage->stolon_config.store_prefix);
		if (copy->stolon_config.store_prefix == NULL)
			goto error;
	}
	if (storage->stolon_config.cluster_name) {
		copy->stolon_config.cluster_name = mcxt_strdup(storage->stolon_config.cluster_name);
		if (copy->stolon_config.cluster_name == NULL)
			goto error;
	}
	if (storage->stolon_config.endpoints) {
		copy->stolon_config.endpoints = mcxt_strdup(storage->stolon_config.endpoints);
